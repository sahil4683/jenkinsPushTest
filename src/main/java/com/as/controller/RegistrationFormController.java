...
import com.as.service.RegistrationFormService;
...
public class RegistrationFormController {
	...
	@PreAuthorize("hasRole('ADMIN')")
	@PostMapping("/save")
	public BaseResponse save(@RequestBody RegistrationDto form,
			Principal principal) {
	...
	@PreAuthorize("hasRole('ADMIN')")
	@PutMapping("/update")
	public BaseResponse update(@RequestBody RegistrationDto form, Principal principal) {
		BaseResponse response =  new ApiValidationUtility().validateAPI(form);
		if(response.getStatus() == 300) {
			return response;
		}
